...
export function useOnboardingProviderStep({
  ...
  const usableAgentEntries = useMemo(() => {
    const agentProviders = getAgentProvidersFromEntries(catalogEntries);
    return [...inventoryEntries.values()].filter(
      (entry) =>
        entry.configured &&
  ...
  async function continueWithCurrentDefault() {
    ...
    const isAgentProvider = getAgentProvidersFromEntries(catalogEntries).some(
